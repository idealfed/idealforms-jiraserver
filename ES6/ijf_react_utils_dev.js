...
renderSelect(inFormKey,item, inField, inContainer)
{
    ...
    //get lookuips
			...
			return (
			  <div style={style}>
					<MuiFormControl style={panelStyle} error={this.state.errored} required={fieldStyle.required} disabled={rOnly}>
					  <MuiInputLabel htmlFor="svalue-helper">{lCaption}</MuiInputLabel>
					  <MuiSelect
						value={this.state.svalue}
						onChange={this.handleChange}
						input={<MuiInput style={fieldStyle} name="svalue" id="svalue-helper" readOnly={rOnly}/>}
					  >
						{this.getMenu()}
					  </MuiSelect>
					  <MuiFormHelperText>{inField.toolTip}</MuiFormHelperText>
					</MuiFormControl>
			  </div>
			);
		  }
		...
		var controlReference = ReactDOM.render(<LocalMuiSelectField />, inContainer);
	 ...
	 renderCardList:function(inFormKey,item, inField, inContainer)
	{
       ...
       //REACT section
		...
		class MuiCard extends React.Component {
			  ...
			  {
				return (<div>
						  <Card style={style} onClick={this.handleDblClick}>
